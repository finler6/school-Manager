...
public class DbMigrator(IDbContextFactory<SchoolManageDbContext> dbContextFactory, DALOptions options)
    : IDbMigrator
{
    ...
    {
        await using SchoolManageDbContext dbContext = await dbContextFactory.CreateDbContextAsync(cancellationToken);
